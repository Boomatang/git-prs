...
pub const ConfigError = error{
    ...
    InvalidDefaultTeam,
    NoDefaultTeam,
} || std.mem.Allocator.Error || std.Io.File.OpenError || std.process.RunError;

pub fn loadConfig(allocator: std.mem.Allocator, io: std.Io, environ_map: *std.process.Environ.Map) ConfigError!Config {
    const config_path = try getConfigPath(allocator, environ_map);
    defer allocator.free(config_path);

    const config_file = std.Io.Dir.openFileAbsolute(io, config_path, .{}) catch |err| {
        if (err == error.FileNotFound) {
            printConfigNotFoundError(io);
    ...
    const file_size = config_file.length(io) catch {
        return ConfigError.InvalidJson;
    };
    if (file_size > 1024 * 1024) return ConfigError.InvalidJson;

    var read_buf: [4096]u8 = undefined;
    var reader = config_file.reader(io, &read_buf);
    const config_contents = reader.interface.readAlloc(allocator, @intCast(file_size)) catch {
        return ConfigError.InvalidJson;
    };
    ...
    const auth_token = getAuthToken(allocator, io) catch |err| {
        for (mine_orgs) |org| {
            allocator.free(org);
        }
...
fn getConfigPath(allocator: std.mem.Allocator, environ_map: *std.process.Environ.Map) ![]const u8 {
    const config_home = if (environ_map.get("XDG_CONFIG_HOME")) |xdg_config|
        try allocator.dupe(u8, xdg_config)
    else blk: {
    ...
    return try std.fs.path.join(allocator, &.{ config_home, "git-prs", "config.json" });
}
...
fn getAuthToken(allocator: std.mem.Allocator, io: std.Io) ConfigError![]const u8 {
    const result = process.run(allocator, io, .{
        .argv = &[_][]const u8{ "gh", "auth", "token" },
    }) catch |err| {
        if (err == error.FileNotFound) {
            printGhNotInstalledError(io);
...
fn printConfigNotFoundError(io: std.Io) void {
    const stderr = std.Io.File.stderr();
    stderr.writeStreamingAll(io, "Config not found. Create ~/.config/git-prs/config.json\n\n") catch {};
    stderr.writeStreamingAll(io, "Example:\n") catch {};
...
fn printGhNotInstalledError(io: std.Io) void {
    const stderr = std.Io.File.stderr();
    stderr.writeStreamingAll(io, "gh CLI not found. Install from https://cli.github.com\n") catch {};
}

fn printNotAuthenticatedError(io: std.Io) void {
    const stderr = std.Io.File.stderr();
    stderr.writeStreamingAll(io, "Not authenticated. Run `gh auth login` first\n") catch {};
}
